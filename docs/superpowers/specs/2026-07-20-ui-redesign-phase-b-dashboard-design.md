...
- The `<main>` content column must be horizontally centered: `max-w-[760px] mx-auto px-[clamp(24px,6vw,64px)] ...`. (**Correction, 2026-07-20:** the reference's own dashboard container has no centering at all — a literal port hugs the left edge with a large empty gap on the right on wide screens. Fixed by adding `mx-auto`. See the Phase A spec's "Standing design principle" section — this applies to every future phase's main content column too, e.g. the Phase C wizard steps.)
- Replace the inline `<header>` block with `<AppHeader rightSlot={...} />`, passing `<ThemeToggle />` and the existing "Log out" button as `rightSlot` (a `<>...</>` fragment). The header itself stays edge-to-edge (not centered) — only the content column below it needs `mx-auto`.
