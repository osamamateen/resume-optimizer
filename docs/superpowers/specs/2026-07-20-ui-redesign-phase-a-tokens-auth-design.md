...
**Every page's content — whether a centered card/modal or a `max-w-*` content column — must actually be horizontally centered in the viewport on wide screens. Never leave it hugging the left edge with dead space on the right**, even if that's what the reference `.dc.html` mockup's literal CSS produces. This was corrected twice in practice, both times because the reference's literal styling technically "centers" only relative to its own frame, not a real wide browser window:

1. **Flex-centered overlays (auth cards, the Phase E template-preview modal):** use real `flex items-center justify-center`. The reference sometimes fakes this with viewport-relative asymmetric padding (`clamp(24px,8vw,120px)` on one side only) that happens to look centered at the frame width it was designed at but reads as left-biased at other widths. Login/Signup were built this way first and had to be fixed to `justify-center` (see the Component changes section above).
2. **`max-w-*` content columns (dashboard, and — by the same logic — the wizard steps in Phase C):** add `mx-auto`. The reference's dashboard container (`max-width:760px;padding:...`) has no centering at all, so a literal port hugs the left edge with a large empty gap on the right on any screen wider than ~760px + padding. The dashboard was built this way first and had to be fixed by adding `mx-auto` to its `<main>` (`app/page.tsx`).

Apply both rules by default to every future phase (C/D/E) rather than waiting for another report — check any new `max-w-*` container for `mx-auto` and any new centered overlay for `justify-center` before considering a screen done. The header/nav bar is the one exception: it's meant to span edge-to-edge with content pinned left/right, not centered.
